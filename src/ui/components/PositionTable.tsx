...
const PositionTable: FC<PositionTableProps> = ({positions, editRequested, deleteRequested}) => {
  ...
  const totalValue = positions.reduce((previousValue, position) => previousValue + (position.quantity * position.price), 0);
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right"><Typography>Target %</Typography></TableCell>
          ...
          {positions.length === 0
            ...
            : positions.map((position) => (
              <TableRow key={position.ticker} sx={{'&:last-child td, &:last-child th': {border: 0}}}>
                ...
                <TableCell align="right"><Typography>{position.target}%</Typography></TableCell>
                <TableCell align="right"><Typography>{calculateCurrentPercentage(position.quantity, position.price).toFixed(2)}%</Typography></TableCell>
                <TableCell align="right">
                  ...
                  <IconButton data-testid="PositionTableDeleteButton" size="small"
                              onClick={() => deleteRequested(position.ticker)}><DeleteOutlineOutlinedIcon/></IconButton>
                </TableCell>
              </TableRow>
            ))
          }
