...
describe('Balancer', () => {
  ...
  it('should distribute balance according to target percentages', () => {
    ...
    expect(result.positions.find(p => p.ticker === 'AAPL')?.quantity).toBe(10);
    expect(result.positions.find(p => p.ticker === 'GOOGL')?.quantity).toBe(15);
  });
  ...
  it('should skip buying positions if balance is less than the price of any position', () => {
    ...
    expect(result.positions.find(p => p.ticker === 'AAPL')?.quantity).toBe(10);
    expect(result.positions.find(p => p.ticker === 'GOOGL')?.quantity).toBe(5);
